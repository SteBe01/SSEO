function [rr_t, vv_t] = TBP(M, m, rr0, vv0, rr1, vv1, t, dt)
%TBP Summary of this function goes here
% M : mass of the Sun [kg]
% m : mass of the Earth [kg]
% rr0 : initial position of satellite wrt Earth [km]   
% vv0 : initial velocity of satellite wrt Earth [km/s] 
...
MU=G*M;
mu=G*m;
% [~, Y1] = ode113(@(t,y) ode_2bp(t,y,MU), tt, y1, options) ;  %computing the coordinates of the trajectory of the Earth (ODE solving)
% 
% 
% rx=rrr1(:,1); %x coordinates of the position vector of Earth (column)
% ry=rrr1(:,2); %y coordinates of the position vector of Earth (column)
% rz=rrr1(:,3); %z coordinates of the position vector of Earth (column)
...
oldcmap = colormap('autumn');                                            %colormap(p, hot); %autumn(100)
